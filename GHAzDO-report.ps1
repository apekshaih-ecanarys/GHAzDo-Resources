...
param(
    ...
    [string]$orgUri = "https://dev.azure.com/BFLDevOpsOrg",
    [string]$project = "SalesOne_Repositories",
    [string]$repository = "LEAD_MANAGEMENT",
    ...
    $alertList += foreach ($alert in $parsedAlerts.value) {
        ...
        if ($alert.severity -in $severities `
            ...
            [pscustomobject]@{
                ...
                "State Changed By" = $alert.dismissal.stateChangedBy
                "State Changed By Identity" = $alert.dismissal.stateChangedByIdentity.displayName
